...
Janus 是 OpenClaw 的完整第二记忆系统，参考 Claude Code 的会话管理 + 粘贴管理 + 上下文管理，三大功能合并为一个统一系统。
...
Janus 是纯 JavaScript 模块，无需安装，直接使用：
...
为了获得完整的记忆体验，推荐将 Janus 与 Dream 和 Lao 一起安装使用：

### 🌙 Dream - 记忆整合系统

Dream 自动将短期记忆整合为长期记忆：
- 每日凌晨 5 点自动整合
- 智能分析与规则修剪
- 变更前自动备份
- 内存优化 50%

**安装位置**：`~/.openclaw/workspace/skills/dream-system/`
...
**安装位置**：`~/.openclaw/workspace/skills/lao-retrieval/`
...
Janus 是 OpenClaw 的内置系统，欢迎提交问题和改进建议。
